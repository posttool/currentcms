...
exports.removeEach = function(list, complete) {
  exports.forEach(list, function (s, n) {
    try {
...
// node cms admin ... & node cms start ...
exports.create_admin = function(domain) {
  ...
  prompt.get(prompt_schema, function (err, result) {
    ...
    var cms = new Cms(domain);
    var User = cms.meta.model('User');
    exports.save_user(new User(), {name: result.name, email: result.email, email_verified: true,
...
exports.start_server = function(domain){
  var app = require('express')();
  var cms = new Cms(domain);
  app.use(cms.app);
  if (domain.app)
    app.use(domain.app(cms.config, cms.meta));
  // if uses_custom_fields(domain)
  //    app.use(express.static(domain.?__dirname + '/public'));
