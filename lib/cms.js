...
Cms.prototype._init = function (with_app) {
  var self = this;
  ...
  // model helper
  self.meta = new meta.Meta(self.module.models, self.connection, with_app);
  ...
  switch (self.config.storage) {
    case "file":
      logger.info('cms file storage ['+self.config.fileConfig.basePath+']');
      break;
    case "pkgcloud":
  ...
  if (self.config.kueAdminPort && self.jobs) {
    kue.app.listen(self.config.kueAdminPort);
  }
  ...
  app.get('/cms/backup',
    middle2, self.show_backup.bind(self));
  app.post('/cms/backup/download',
    middle2, self.backup_download.bind(self));
  app.post('/cms/backup/upload',
    middle2, self.backup_upload.bind(self));
  app.post('/cms/backup/progress',
    middle1, self.backup_progress.bind(self));
...
Cms.prototype.form_post = function (req, res) {
  ...
  if (!presave)
    return do_save();
  var args = presave.toString().match(/^\s*function\s+(?:\w*\s*)?\((.*?)\)/);
  args = args ? (args[1] ? args[1].trim().split(/\s*,\s*/) : []) : null;
  if (args.length == 4)
    presave(self, object, data, do_save);
  else
    presave(object, data, do_save);
};
...
Cms.prototype.resource_upload = function (req, res) {
  ...
  form.onPart = function (part) {
    ...
    var path = uuid.v4().substring(0,6) + '-' + part.filename;
...
//  });
  ...
  switch (self.config.storage) {
    case "file":
      var p = self.config.fileConfig.basePath + '/' + path;
...
Cms.prototype.delete_resource = function (id, complete) {
  ...
  q.exec(function (err, r) {
    ...
    if (r) {
      switch (self.config.storage) {
        case "file":
          var p = self.config.fileConfig.basePath + '/' + r.path;
          fs.unlink(p, complete);
          break;
        case "pkgcloud":
          self.client.removeFile(self.config.container, r.path, function (err) {
            if (err) return complete(err);
            // children too!
    ...
    if (r) {
      switch (self.config.storage) {
        case "file":
          ...
          // res.sendfile(self.config.resourcePath + r.path);
          break;
        case "pkgcloud":
          break
        case "cloudinary":
...
Cms.prototype.show_backup = function (req, res) {
  //var self = this;
  res.render('cms/backup', {
    title: 'Backup',
    models: req.models,
    jsIncludes: req.form_includes
  })
...
Cms.prototype.backup_download = function (req, res) {
  var self = this;
  // create zip , send to browser
  // transform ids?
  // order by create
  // 1 file per row or 1 csv per type
};

Cms.prototype.backup_upload = function (req, res) {
  var self = this;
      //var file = fs.createWriteStream(self.config.resourcePath + path);
  ...
  // UNZIP
  // for each file, match type by file name
  //   remove all existing elements & restore

};

Cms.prototype.backup_progress = function (req, res) {
  var self = this;
  // unzipping
  // deleting, inserting...
};
