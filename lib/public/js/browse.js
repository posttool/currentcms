function browse_browse(app, type, f) {
  ...
  function create_row(r) {
    ...
    for (var j = 0; j < bmeta.length; j++) {
      ...
      var $c = $$('ccol nowrap');
      $c.css({width: p});
      // to do handle cell types same way form does... pluggable ples
      if (b.cell == 'image')
      {
        var u = find_thumb2(r);
        if (u)
        ...
        var ref = schema.schema[b.name].ref;
        var arr = schema.schema[b.name].is_array;
        if (ref){
          var t = render_template(ref, arr ? ((v && v.length != 0) ? v[0] : '') : v).trim();
          $c.text(t);
        } else
          $c.text(v);
      }
