...
function form_form(app, type, id) {
  ...
  var $el = mixin_basic_component(self, 'form');
  mixin_emitter(self);
  ...
  function update_info() {
    ...
    // rd.add_listener('select', function (f, o) { self.emit('select', o); });
    // rd.add_listener('close', function () { self.emit('close'); });
    rd.bubble_listener(self);
    $info.append(rd.$el());
...
function indicated_field(d)//, settings_callback)
{
  ...
  var $el = mixin_basic_component(self, 'control-group '+type+' '+label);
  mixin_emitter(self);
